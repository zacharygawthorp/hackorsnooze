...
function generateStoryMarkup(story, showDeleteBtn = false) {
  ...
  // if the user is logged in, show fav/non-fav star
...
/** Make fav/non-fav star for story */
...
/** Create data when submitting the form */
...
function putUserStoriesOnPage() {
  ...
  if (currentUser.ownStories.length === 0) {
    $myStories.append("<h5>No stories added yet!</h5>");
  } else {
...
async function toggleStoryFav(evt) {
  ...
  // see if item is already fav(checking presence of star)
  if($target.hasClass("fas")) {
    //currently a fav: remove from user's fav list and change star
    await currentUser.removeFav(story);
    $target.closest("i").toggleClass("fas far");
  } else {
    //currently not a fav: do the opposite
